      ...
      <form onSubmit={handleSubscribe}>
        <div className="flex flex-col sm:flex-row">
          <div className="relative w-full">
            <input
              className={cn(
                "button-label-2 w-full flex-grow rounded-large p-3 pl-[2.7rem]",
                inputClass
              )}
            ...
            />
            <Envlope
              className="absolute left-3 top-[50%] h-6 w-6 translate-y-[-50%]"
              color={globeColorClass}
            />
          </div>
        ...
        <div>
          {!isSubscribed && error && (
            <p className="body-3 mt-2 text-light-status-critical-1 dark:text-dark-status-critical-1">
              {error}
            </p>
          )}
          {isSubscribed && (
